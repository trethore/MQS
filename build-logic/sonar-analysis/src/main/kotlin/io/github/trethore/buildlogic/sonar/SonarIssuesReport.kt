...
internal data class SonarIssuesReport(val projectKey: String, val issues: List<SonarIssue>)
...
internal class SonarIssuesLoader(private val client: SonarClient) {
    fun load(projectKey: String): SonarIssuesReport {
        val issues = mutableListOf<SonarIssue>()
        var page = 1
        while (true) {
            if (page > SonarConstants.MAX_PAGES) {
                throw GradleException("SonarQube issues response exceeded ${SonarConstants.MAX_PAGES} pages.")
            }
            ...
            pageIssues.mapTo(issues) { sonarIssue(projectKey, it) }
            if (issues.size >= total) return SonarIssuesReport(projectKey, issues)
            if (pageIssues.isEmpty()) {
                throw GradleException("SonarQube returned an empty issues page before the reported total was reached.")
            }
            page += 1
        }
    }
