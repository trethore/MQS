...
internal class SonarCoverageLoader(
    ...
    fun load(projectKey: String): SonarCoverageReport {
        ...
        val files = if ((uncoveredLines ?: 0) > 0) fetchFiles(projectKey) else emptyList()
    ...
    private fun coverageFile(component: Map<*, *>): SonarCoverageFile? {
        ...
        if (uncoveredLines <= 0) return null
        ...
        val path = component["path"]?.toString()
            ...
            ?: "unknown"
        return SonarCoverageFile(path, measures[LINE_COVERAGE], linesToCover - uncoveredLines, linesToCover)
    }
}
