...
final class JavaInteropService {
  ...
  JavaInteropMembers getMembers(Class<?> targetClass, String preferredNamedClassName) {
    ...
    return memberCache.computeIfAbsent(
        cacheKey,
        ignored -> new JavaInteropMembers(targetClass, preferredNamedClassName, mappings));
  }
  ...
  private static IllegalStateException invocationFailure(
    ...
    Throwable cause =
        exception instanceof InvocationTargetException invocationTargetException
  ...
  private String resolveNamedClassName(Class<?> runtimeClass, String preferredNamedClassName) {
    if (preferredNamedClassName != null) {
      MappingIndex.ClassMapping preferredMapping =
          mappings.getClassMapping(preferredNamedClassName);
