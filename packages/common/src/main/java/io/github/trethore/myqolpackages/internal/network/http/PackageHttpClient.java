...
public final class PackageHttpClient implements AutoCloseable {
  ...
  public synchronized CompletableFuture<PackageHttpResponse> send(PackageHttpRequest request) {
    ...
    if (closed) {
      return CompletableFuture.failedFuture(
          new IllegalStateException("Package HTTP client is closed"));
    }
    if (requests.size() >= MAX_PENDING_REQUESTS) {
      return CompletableFuture.failedFuture(
          new IllegalStateException("Too many pending HTTP requests"));
    }
  ...
  private static PackageHttpRequest redirectedRequest(
      PackageHttpRequest request, URI redirectUri, int statusCode) {
    boolean switchToGet =
        statusCode == 303
            || ((statusCode == 301 || statusCode == 302) && request.method().equals("POST"));
    boolean sameOrigin = hasSameOrigin(request.uri(), redirectUri);
    List<PackageHttpHeader> headers =
    ...
    if (request.hasBody()) {
      return PackageHttpRequest.withBody(
          redirectUri, request.method(), headers, request.bodyData());
    }
  ...
  private static PackageHttpResponse readResponse(
